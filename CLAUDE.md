...
- **Ecom_Funnel_Architecture.md** — Ce qui se passe après le clic : la congruence créative → funnel, le hook comme mécanisme de ciblage algorithmique et la couverture Mood × Intensité, les trois mécaniques biologiques d'un hook, **le copywriting complet (parler pas écrire, les 3 checks visualisable/falsifiable/unique, faits vs opinions, croyances, autorité en 3 niveaux et 10 leviers, open loop, le Lead en 6 techniques, montrer ne pas dire, l'enjeu en 5 leviers, gérer les objections, le Self-Concept en 3 angles)**, le parcours émotionnel (Primal → Awareness → Interest → Desire → Action), l'advertorial et ses 5 formats, la landing page (above the fold, carrousel, réassurance), et les 7 mécaniques d'offre avec les 3 niveaux. Référence des skills `funnel` et `offer` en e-commerce. Contient une section « Réserves ».
